...
export class AppModel extends Observable {
  ...
  private onFirebaseReady() {
    ...
    firebase.addValueEventListener(result=>{
      ...
      this.set('connected', result.value);
    }, ".info/connected");

    
    firebase.addValueEventListener(result=>this.onStationsUpdate(result.value), "/stations");
    firebase.keepInSync(
      "/stations", // which path in your Firebase needs to be kept in sync?
